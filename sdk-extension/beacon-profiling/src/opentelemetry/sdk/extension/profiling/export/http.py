...
class OTLPHTTPProfileExporter:
    ...
    def export(
        ...
        for retry_num in range(_MAX_RETRYS):
            ...
            try:
                response = self._export(serialized_data, deadline_sec - time())
                if response.ok:
                    return ProfileExportResult.SUCCESS
            except requests.exceptions.RequestException as error:
                reason = error
                retryable = isinstance(error, RequestsConnectionError)
                status_code = None
            else:
            ...
            if (
                ...
                _logger.error(
                    "Failed to export profile batch code: %s, reason: %s",
                    status_code,
                    reason,
                )
                return ProfileExportResult.FAILURE
            _logger.warning(
                "Transient error %s encountered while exporting profile batch, retrying in %.2fs.",
                reason,
                backoff_seconds,
            )
...
def _append_profiles_path(endpoint: str) -> str:
    if endpoint.endswith("/"):
        return endpoint + DEFAULT_PROFILES_EXPORT_PATH
    return endpoint + f"/{DEFAULT_PROFILES_EXPORT_PATH}"
